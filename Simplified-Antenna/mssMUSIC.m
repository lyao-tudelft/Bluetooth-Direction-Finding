function response = mssMUSIC( parameter, X )
% MUSIC algorithm with modified spatial smoothing for angle estimation of 
% coherent signals
%
...
for l = 1:L
    ...
    Rx_hat(:,:,l) = J*conj(Rx(:,:,l))*J;
end
Rx_hat_smoothed = sum(Rx_hat,3)/L;
Rx_smoothed = (sum(Rx,3)+sum(Rx_hat,3))/(2*L);
...
grid on;
title('MSS-MUSIC AoA Estimation');
xlabel('angle/degree');
